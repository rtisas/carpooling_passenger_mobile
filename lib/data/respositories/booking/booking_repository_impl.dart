...
class BookingRepositoryImpl implements BookingRepository {
  ...
  @override
  Future<Either<Failure, BookingResponse>> createBookingRequest(
      BookingRequest bookingRequest) async {
    try {
      final bookingResponse =
          await _bookingRemoteDataSource.createBooking(bookingRequest);
  ...
  @override
  Future<Either<Failure, List<BookingResponseComplete>>>
      getBookingsPassengerByState(String idPassenger, String status) async {
    try {
      final listBookingResponse = await _bookingRemoteDataSource
          .getBookingsPassengerByState(idPassenger, status);
  ...
  Future<Either<Failure, dynamic>> deleteBooking(String idBooking) async {
    try {
      final listBookingResponse =
          await _bookingRemoteDataSource.deleteBooking(idBooking);
      return Right(listBookingResponse);
    } on DataIncorrect {
      return Left(FailureResponse(
          message: 'Algo salió mal, por favor verifique los datos'));
  ...
  @override
  Future<Either<Failure, BookingResponseComplete>> getBookingsById(
      String idBooking) async {
    try {
      final bookingResponse =
          await _bookingRemoteDataSource.getBookingById(idBooking);
      return Right(bookingResponse);
    } on DataIncorrect {
      return Left(FailureResponse(
          message: 'Algo salió mal, por favor verifique los datos'));
      ...
      return Left(FailureResponse(
          message: 'La reserva no existe o no se encuentra disponible'));
      // return Left(FailureResponse(message:'Aún no hay reservas disponibles'));
    } on ServerException {
      return Left(FailureResponse(
          message:
  ...
  @override
  Future<Either<Failure, void>> updateQualifying(
      String idBooking, UpdateQualifying updateQualifying) async {
    try {
      final updateQualifiyingResponse = _bookingRemoteDataSource
          .updateQualifyingBooking(idBooking, updateQualifying);
      return Right(updateQualifiyingResponse);
    } on DataIncorrect {
      return Left(FailureResponse(
          message: 'Algo salió mal, por favor verifique los datos'));
    } on NoValidRole {
